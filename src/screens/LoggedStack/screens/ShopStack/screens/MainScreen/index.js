...
import PlusIcon from '@assets/media/plus.svg';
...
const MainScreen = ({navigation}) => {
   ...
   const { productList } = useContext(ProductListContext);
    ...
    const itemRender = (item) => {
        return (
            <View style={{padding: 16, marginBottom: 15, borderWidth: 1, borderColor: 'rgba(0, 0, 0, 0.1)', borderRadius: 8, flex: 1}}> 
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                    <View>
                        <Text style={{color: '#000', lineHeight: 16, fontSize: 16, fontFamily: 'PlusJakartaSans-Bold', letterSpacing: -.29}}>{item.name}</Text>
                        <TouchableOpacity onPress={() => addToCart(item)} activeOpacity={.9} style={{width: 109, height: 36, marginTop: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#DB2B36', borderRadius: 6}}>
                            <View style={{marginRight: 5}}><PlusIcon/></View>
                            <Text style={{color: '#fff', lineHeight: 13, fontSize: 13, opacity: 1, fontFamily: 'PlusJakartaSans-Medium', letterSpacing: -.29}}>{t('logged.mainscreen.add_to_card')}</Text>
                        </TouchableOpacity>
                    </View>
                    <View>
    ...
    return (
        <View style={{backgroundColor: '#fff', flex: 1}}>
            <LoggedHeader navigation={navigation} title={t('logged.mainscreen.products')} description="Page description"/>
            <MiniCard/>
                ...
                <View style={{backgroundColor: '#fff', paddingBottom: 40, borderTopLeftRadius: 10, borderTopRightRadius: 10}}>
                    <View style={{flexDirection: 'row', paddingHorizontal: 20, justifyContent: 'space-between', alignItems: 'center'}}>
                        <View style={{borderBottomWidth: 1, marginTop: 10, flex: 1, borderBottomColor: '#e5e5e5'}}>
                            <Text style={{color: '#000', paddingVertical: 15, lineHeight: 16, fontSize: 16, opacity: 1, fontFamily: 'PlusJakartaSans-Bold', letterSpacing: -.29}}>{t('logged.mainscreen.add_to_card')}</Text>
                        </View>
                        ...
                        <View style={{marginTop: 15}}>
                            <View>
                                ...
                                <TouchableOpacity style={{
                                    ...
                                    <Text style={{fontFamily: 'PlusJakartaSans-Bold', lineHeight: 16, fontSize: 16, color: '#fff'}}>{t('logged.mainscreen.add_to_card')}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    </View>
